...
export class PhoneticEditor extends HTMLElement {
  ...
  async loadData() {
    try {
      ...
      this.showError('Failed to load phonetic data. Please check your connection.');
    }
  }
  ...
  saveRulesToStorage() {
    localStorage.setItem('phoneticEditorRules', JSON.stringify(this._substitutionRules));
  }
  ...
  setupEventListeners() {
    ...
    if (addRuleButton) {
      addRuleButton.addEventListener('click', () => {
        ...
        const replacementInput = this.shadowRoot.querySelector('.replacement-input');
        
        if (patternInput && replacementInput && patternInput.value && replacementInput.value) {
    ...
    const searchFeaturesButton = this.shadowRoot.querySelector('.search-features-button');
  ...
  selectTab(index) {
    ...
    this.shadowRoot.querySelectorAll('.tab-panel').forEach(panel => {
      panel.classList.toggle('active', parseInt(panel.dataset.panelIndex) === index);
    });
  }

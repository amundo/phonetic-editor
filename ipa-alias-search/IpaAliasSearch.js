...
class IpaAliasSearch extends HTMLElement {
  ...
  renderSearchResults(results) {
    ...
    if (resultCount) {
      ...
      resultCount.textContent = `Found ${uniqueCount} unique character${uniqueCount !== 1 ? 's' : ''}`;
    }
  }
  ...
  hideLoading() {
    ...
    const resultsContainer = this.querySelector('.search-results');
    if (resultsContainer) {
      resultsContainer.innerHTML = '<div class="no-results">Start typing to search characters</div>';
    }
  }
